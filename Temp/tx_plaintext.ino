 ...
 * Sends a fixed 16-byte message every TEST_INTERVAL_MS.
 * No encryption — ENC_US is always 0.
 */
...
void loop() {
    ...
    // No encryption — time is zero
    unsigned long enc_us = 0;

    driver.send(buf, MSG_SIZE);
    driver.waitPacketSent();

    // Structured output for Python collector
