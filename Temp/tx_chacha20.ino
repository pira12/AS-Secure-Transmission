 ...
 * Minimal AVR-friendly ChaCha20 implementation (no external library).
 * ChaCha20 is a stream cipher — no padding, no block alignment needed.
 * Nonce is static here for simplicity; production use must increment.
 *
 * Protocol: TEST,CHACHA20,<size>,ENC_US:<t>,ITER:<n>
 *
 * Reference: RFC 7539
 * This implementation works on AVR 8-bit with no 64-bit arithmetic.
 */
...
// 96-bit nonce (12 bytes) — static for demo, increment in production
...
// Generates one 64-byte keystream block from the ChaCha20 state
...
// XOR plaintext with keystream (stream cipher encryption)
